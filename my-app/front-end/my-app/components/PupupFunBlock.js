import React from "react";
import './../assets/style/main.css';

const PupupFunBlock = ({ showPage, sShowPopupFB }) => {
  return (
    <div id="fondonero">
      <table id="popup">
        <tbody>
            <tr>
                <td><img onClick={() => sShowPopupFB(false)} src="data:image/png;base64,"/></td>
                <td><h2 className="titoli">Funzionalita bloccata<br />Devi registrarti/accedere per continuare</h2></td>
            </tr>
            <tr><td collspan="2"><h2 onClick={() => {showPage(5);sShowPopupFB(false);}} className="link">Registrati</h2></td></tr>
            <tr><td collspan="2"><h2 onClick={() => {showPage(4);sShowPopupFB(false);}} className="link">Accedi</h2></td></tr>
        </tbody>
      </table>
    </div>
  );
}
export default PupupFunBlock;